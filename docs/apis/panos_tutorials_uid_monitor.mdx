---
id: panos_tutorials_uid_monitor
...
Although they cover similar objectives there are fundamental technical differences between them:
...
Years ago it was up to the end customer to create its own connectors (scripts) to link application
logs containing IP metadata with its NGFW's. Implementing CGI scripts on a WEB server having RO access
to these logs and publishing them as IP lists (EDL's) was, by far, the easiest way to go.

But as presence of PAN-OS powered NGFW's grown among enterprise customers, more and more application
vendors decided to provide User-ID connectors out-of-the-box. Althought this is great (from a feature
point of view) losing the option to fetch a list from a URL might make it difficult to integrate
legacy technologies.
...
One option would be to leverage a PAN-OS XML SDK like [PAN Python](https://panos.pan.dev/docs/apis/panpython_qs)
or [PAN GO](https://panos.pan.dev/docs/apis/pango_qs) and to create a REST API that extracts the
...
enforcing timeout (adding timeout to entries that do not have them), converting DAG messages
