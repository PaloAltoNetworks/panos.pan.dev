...
This is not required, but we suggest using [virtualenv](https://virtualenv.pypa.io/en/latest/) to keep ur test and development environments clean. It's super easy:

```shell
pip install virtualenv
```

Once installed, you can use the `virtualenv` command to create a new virtual environment to store your ckages without polluting the system library:
...
After the end of the tutorial you can remove Ansible and all the installed packages by removing the rtualenv directory:
```console
...
We can now the playbook using `ansible-playbook`:
...
> when `--ask-vault-pass` is used, `ansible-playbook` will assk for the password to decrypt the `vars.yml` file
