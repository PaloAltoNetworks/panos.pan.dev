...
|-------------|-----------------------------------------------------------------|------------|
|       POST  | <small>`https://<ExpIP>/api/v1/external-tools/convert`</small>  | <small>_in body_<br/>{**"input"**: "path to the ZIP file",<br/>**"output"**: "path to the resulting PANOS config file"}</small>|
|    example  | <small>`https://10.0.0.1/api/v1/external-tools/convert`</small> | <small>_in body_<br/>{**"input"**: "/var/www/html/expedition-api/contents/configSamples/migration1.zip",<br/>**"output"**:"/tmp/convertedConfig.xml" }</small>|
