...
| Method | EndPoint                                   | Parameters | Example Value                                                    |
|-------------|--------------------------------------------|------------|------------------------------------------------------------------|
|       POST     | `https://<YourExpeditionIP>/api/v1/device/keys` |            |{**"id"**: "deviceID",<br/>**"role"**: "admin",<br/> **"username"**: *"YourPAN-OSdeviceLoginusername"*, <br/>**"password"**:*"YourPAN-OSdeviceLoginPassword"*} |  
...
| Method | EndPoint                                                                       | Parameters                                                              | Example Value                           |
|-------------|--------------------------------------------------------------------------------|-------------------------------------------------------------------------|-----------------------------------------|
|     GET     | `https://<YourExpeditionIP>/api/v1/device/{deviceId}/retrieveContent/{config}`<br/><br/> With **{deviceID}**:"1" **{config}**:"running"<br/><br/>API EndPoint Full path will become: <br/><br/>*`https://<YourExpeditionIP>/api/v1/device/1/retrieveContent/running`*  | **"deviceID"**-> PAN-OS device ID,<br/>**"config"**-> "running"or "candidate" |   |  
...
| Method | EndPoint                                                                       | Parameters                                                              | Example Value                           |
|-------------|--------------------------------------------------------------------------------|-------------------------------------------------------------------------|-----------------------------------------|
|     POST     | `https://<YourExpeditionIP>/api/v1/projects/{projectId}/apiCalls/{type}`<br/><br/>  With sample parameter value: **{projectID}**:"$projectId", **{type} **:"atomic"<br/><br/>  API EndPoint Full path will become: <br/><br/>*`https://<YourExpeditionIP>/api/v1/projects/1/apiCalls/atomic`*   | **"projectId"**-> Expedition Project ID,<br/><br/>**"type"**-> "mega" or "atomic" or "subatomic" or "clear" | {**"serial"**: serial#ofyourFW",<br/>**"role"**: "admin",<br/>**"auth_type"**: "UserPassword",<br/>**"username"**: *"YourPAN-OSdeviceLoginusername"*,<br/>**"password"**: *"YourPAN-OSdeviceLoginpassword"*,<br/>**"sourceId"**: 0,<br/>**"action"**: "set"<br/>} |  
...
data = {"project":projectId,"baseConfig":panosConfigPath,"source":source}
r = requests.post('https://'+ip+'/api/v1/apiCalls/atomic', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
...
| Method | EndPoint                                                                       | Parameters                                                              | Example Value                           |
|-------------|--------------------------------------------------------------------------------|-------------------------------------------------------------------------|-----------------------------------------|
|     POST    | `https://<YourExpeditionIP>/api/v1/projects/{projectId}/device/push` | {**"projectId"**:"$projectId",<br/> With sample parameter value: **{projectID}**:"1", **{type} **:"atomic"<br/><br/>  API EndPoint Full path will become: <br/><br/>*`https://<YourExpeditionIP>/api/v1/projects/1/apiCalls/atomic`*   | **"projectId"**-> Expedition Project ID,<br/><br/>**"type"**-> "mega" or "atomic" or "subatomic" or "clear" | {**"serial"**: serial#ofyourFW",<br/>**"role"**: "admin",<br/>**"auth_type"**: "UserPassword",<br/>**"username"**: *"YourPAN-OSdeviceLoginusername"*,<br/>**"password"**: *"YourPAN-OSdeviceLoginpassword"*,<br/>**"sourceId"**: 0,<br/>**"action"**: "set"<br/>} |  
...
```python
data = {"serial":serial,"project":projectId,"source":source}
r = requests.post('https://'+ip+'/api/v1/device/push', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
