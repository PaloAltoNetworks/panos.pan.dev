...
|-------------|----------------------------------------------|--------------------------------|
|    POST     | <small>`https://<ExpIP>/api/v1/project`</small> | <small>_in url_<br/>{ **"project"**:"project1", **"description"**:"Project for testing" }</small> |  
|    example  | <small>`https://10.0.0.1/api/v1/project`</small> | <small>{"project":"MyLittleProject", "description":"A migration project"}</small>           | 
...
r = requests.post('https://'+ip+'/api/v1/project', data=data, verify=False, headers=hed)
response=r.json()
...
The configurations to be migrated must be contained in a Zip file that provides the different configuration files and 
some instructions on how the migration needs to be performed.
More information about this Zip creation can be found in [Creating a migration file](creating_migration_zip.mdx) for a 
manual Zip creation and in [Creating a migration file (by steps)](creating_migration_zip_2.mdx) for an assisted Zip creation.

(CASE A) In case of performing the Zip manually, make sure you:
- name the Zip such as `zipArchive_<resourceId>.zip` in where `resourceId` is a numeric value you would later use to refer to the zip resourceId.
For instance, _zipArchive_1234.zip_.
...
Refer to [Creating a migration ZIP file](creating_migration_zip) to see the list of supported vendors

API syntax for Converting 3rd party vendors' configurations:  

| Method      | URL                                                        | Parameters |
|-------------|------------------------------------------------------------|------------|
|       POST  | <small>`https://<ExpIP>/api/v1/migration/convert`</small>  | <small>_in body_<br/>{**"resource"**: "Zip resource id",<br/>(optional) **dry-run**: boolean}</small>|
|    example  | <small>`https://10.0.0.1/api/v1/migration/convert`</small> | <small>_in body_<br/>{"resource": 1234,<br/> dry-run: true }</small>|
...
r = requests.post(
        'https://'+ip+'/api/v1/migration/convert', 
        data=data, 
        verify=False, 
        headers=hed)
response=r.json()
...
if success=='true':
    ...
    print(
      json.dumps(
         response['Contents']['response']['response-messages']['messages'][0]['message']))
print('')
```
</TabItem> 
<TabItem value="php"> 
...
func main() {Creating an Expedition project
    //TODO
}
```
</TabItem> 
...
| Method      | EndPoint                                   | Parameters | 
|-------------|--------------------------------------------|------------|
|       POST  | <small>`https://<ExpIP>/api/v1/project/{projectId}/import`</small> | <small>_in url_<br/>**"project"**: "projectID"<br/><br/>_in body_<br/>{ **"resource"**: "configuration resource id" }</small>|
| example     | <small>`https://10.0.0.1/api/v1/project/1/import`</small>         | <small>_in url_<br/>project=1<br/><br/>_in body_<br/>{ "resource": "43232" }</small> |
...
data = {"resource":converterConfig}
r = requests.post('https://'+ip+'/api/v1/project/'+projectId+'importConfig', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
...
$url = 'https://'.$ip.'/api/v1/project/'.$projectId.'/importConfig';
$curl = curl_init($url);
curl_setopt($curl, CURLOPT_HTTPHEADER,$hed);
curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($curl,CURLOPT_POST, TRUE);
...
| Method | EndPoint                                                          | Parameters                                                              | 
|-------------|--------------------------------------------------------------|-------------------------------------------------------------------------|
|     PUT     | <small>`https://<YourExpeditionIP>/api/v1/project/`</small> | <small>_in body_<br/>{**"devices"**:["deviceID1", "deviceID2"], <br/>**"id"**:"projectID"}</small> |  
|     example |  <small>`https://10.0.0.1/api/v1/project/`</small>          |  <small><br/>{"devices": [1],<br/>"id": 1}</small>|
...
r = requests.put('https://'+ip+'/api/v1/project', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
...
| Method | EndPoint                                                                                    | Parameters                                                              | 
|-------------|----------------------------------------------------------------------------------------|-------------------------------------------------------------------------|
|     POST    | <small>`https://<YourExpeditionIP>/api/v1/project/{projectId}/apiCalls/{type}`</small>| <small>_in url_<br/>**"projectId"**-> Expedition Project ID,<br/>**"type"**-> "mega" or "atomic" or "subatomic" or "clear"<br/><br/> _in body_<br/>{**"serial"**: serial#ofyourFW",<br/>**"role"**: "admin",<br/>**"auth_type"**: "UserPassword",<br/>**"username"**: *"YourPAN-OSdeviceLoginusername"*,<br/>**"password"**: *"YourPAN-OSdeviceLoginpassword"*,<br/>**"sourceId"**: 0,<br/>**"action"**: "set"<br/>}</small> |  
|     example | <small>`https://10.0.0.1/api/v1/project/1/apiCalls/atomic`</small>          | <small>{"serial": 012123212,<br/>"role":"admin",<br/>"auth_type":"UserPassword",<br/>"username":"YourPAN-OSdeviceLoginusername",<br/>"password":"YourPAN-OSdeviceLoginpassword",<br/>"sourceId":0,<br/>"action":"set"<br/>}</small> |  
...
data = {"source":source}
r = requests.post('https://'+ip+'/api/v1/project/'+projectId+'apiCalls/atomic', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
...
$url = 'https://'.$ip.'/api/v1/project/'.$projectId.'/apiCalls/atomic';
$curl = curl_init($url);
curl_setopt($curl, CURLOPT_HTTPHEADER,$hed);
curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($curl,CURLOPT_POST, TRUE);
...
API syntax for sending APIcalls to the PAN-OS Device:  
 ...
 Method       | EndPoint                                                                              | Parameters                                                              | 
|-------------|---------------------------------------------------------------------------------------|-------------------------------------------------------------------------|-----------------------------------------|
|     POST    | <small>`https://<YourExpeditionIP>/api/v1/project/{projectId}/device/push`</small>   | <small>_in url_<br/>**"projectId"**-> Expedition Project ID<br/><br/>_in body_<br/>{**"serial"**: serial#ofyourFW",<br/>**"source"**: PAN-OSconfigfile<br/>}</small> | 
|     example | <small>`https://10.0.0.1/api/v1/project/1/device/push`</small>             | <small>_in url_<br/>"projectId"= 1<br/><br/>_in body_<br/>{"serial": 01213221,<br/>"source":1<br/>}</small> | 
...
r = requests.post('https://'+ip+'/api/v1/project/'+projectID+'/device/push', data=data, verify=False, headers=hed)
response=r.json()
success = json.dumps(response['Contents']['success'])
if success=='true':
...
$url = 'https://'.$ip.'/api/v1/project/'.$projectId.'/device/push';
$curl = curl_init($url);
curl_setopt($curl, CURLOPT_HTTPHEADER,$hed);
curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($curl,CURLOPT_POST, TRUE);
...
| Method | EndPoint                                                                       | Parameters                                                              |
|-------------|--------------------------------------------------------------------------------|-------------------------------------------------------------------------|
|     POST    | <small>`https://<ExpIP>/api/v1/project/{projectId}/export`</small> | <small>_in url_<br/>{**"projectId"**->"$projectId"<br/><br/>_in body_<br/>{**"out:"**->PAN-OS configuration file,<br/>**"sourceID:"**->source configuration ID<br/>}</small> |  
|     POST    | <small>`https://10.0.0.1/api/v1/project/1/export`</small> | <small>_in url_<br/>"projectId"=1<br/><br/>_in body_<br/>{"out:"->"toExport.xml",<br/>"sourceID:"1}</small> |  
...
$url = 'https://'.$ip.'/api/v1/project/'.$projectId.'/export';
$curl = curl_init($url);
curl_setopt($curl, CURLOPT_HTTPHEADER,$hed);
curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($curl,CURLOPT_POST, TRUE);
